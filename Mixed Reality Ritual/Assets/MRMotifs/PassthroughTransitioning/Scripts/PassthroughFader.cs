...
namespace MRMotifs.PassthroughTransitioning
{
    ...
    public class PassthroughFader : MonoBehaviour
    {
        ...
        [SerializeField] private GameObject introVrWorld;   // drag INTRO_VR_WORLD here in Inspector
        [SerializeField] private bool startInPassthrough = true;
        [SerializeField] private bool disableVrWorldWhenInPassthrough = true; // if true, VR world turns off when MR is on
        ...
        private void Awake()
        {
            ...
            {
                // Start in MR (passthrough visible)
                m_oVRPassthroughLayer.enabled = true;
                m_targetAlpha = 1f;
                m_material.SetFloat(s_invertedAlpha, 1f);

                // If you're in Underlay mode, make background transparent immediately
                ...
                // Turn OFF VR world at startup (so you only see passthrough)
        ...
        public void TogglePassthrough()
        {
            ...
            switch (State)
            {
                case FaderState.MR:
                    ...
                    // We are leaving MR and going into VR
                    if (introVrWorld != null)
